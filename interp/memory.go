...
func (mv *memoryView) markExternal(llvmValue llvm.Value, mark uint8) {
	...
	global := getUnderlyingGlobal(llvmValue)
	if !global.IsAGlobalVariable().IsNil() {
		objectIndex := mv.r.getValue(global).(pointerValue).index()
	...
	} else if !global.IsAConstantInt().IsNil() {
		// This is a constant int, perhaps as used in memory-mapped I/O.
		// There is nothing to mark.
		return
	} else {
		panic("unknown value to mark as external")
	}
}
...
// getUnderlyingGlobal will return the global behind a pointer value. If it
// can't find the global, it panics.
func getUnderlyingGlobal(v llvm.Value) llvm.Value {
	if !v.IsAGlobalVariable().IsNil() {
		// This is a global, we're done.
		return v
	}
	if !v.IsAConstantInt().IsNil() {
		// This is not a global, but return it anyway because this is as close
		// as we get to one.
		return v
	}
	// Assume an instruction or constant expression.
	var opcode llvm.Opcode
	if !v.IsAInstruction().IsNil() {
		opcode = v.InstructionOpcode()
	} else if !v.IsAConstantExpr().IsNil() {
		opcode = v.Opcode()
	} else {
		v.Dump()
		println()
		panic("interp: cannot find underlying global (unknown value)")
	}
	switch opcode {
	case llvm.GetElementPtr, llvm.BitCast:
		return getUnderlyingGlobal(v.Operand(0))
	case llvm.IntToPtr:
		return getUnderlyingGlobal(v.Operand(0))
	default:
		v.Dump()
		println()
		panic("interp: cannot find underlying global (unknown expression)")
	}
}
